...
public class    JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    private static final String[] NO_CHECK_URL = {"/login","api/lv0/**"}; // "/login"으로 들어오는 요청은 Filter 작동 x
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException{
        ...
        String refreshToken = jwtService.extractRefreshToken(request)
                .filter(jwtService::isTokenValid) //유효한 리플래시 토큰 반환
                .orElse(null); // 유효하지 않거나 존재하지 않으면 NULL을 반환
    ...
    public void checkRefreshTokenAndReIssueAccessToken(HttpServletResponse response, String refreshToken){
        userRepository.findByRefreshToken(refreshToken)
                .ifPresent(userEntity ->{
                    ...
                    jwtService.sendAccessAndRefreshToken(response, jwtService.createAccessToken(userEntity.getIdx(), userEntity.getNickname(),userEntity.getUserProfile()),
